...
def test_gru_forward():
    ...
    expected_results = np.load(os.path.join('autograder',
...
def test_gru_inference():
    ref_outputs = np.load(os.path.join('autograder',
    ...
    student_net = gru_classifier.CharacterPredictor(FEATURE_DIM, HIDDEN_DIM, NUM_CLASSES)
    student_net.init_rnn_weights(w_hi, w_hr, w_hn, w_ii, w_ir, w_in)

    student_outputs = gru_classifier.inference(student_net, inputs)
