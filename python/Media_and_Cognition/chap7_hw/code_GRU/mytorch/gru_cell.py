...
@Description: 
@Author: HuYi
@Date: 2020-06-11 21:33:14
@LastEditors: HuYi
@LastEditTime: 2020-06-12 21:11:05
'''
import numpy as np
...
class GRU_Cell:
    ...
    def backward(self, delta):
        ...
        # (h, h)
        self.dWzh = dz_inner.reshape(-1, 1).dot(self.hidden.reshape(1, -1))
        # (h, d)
        self.dWzx = dz_inner.reshape(-1, 1).dot(self.x.reshape(1, -1))
        ...
        # (h, d)
        self.dWx = dh_tilde_inner.reshape(-1, 1).dot(self.x.reshape(1, -1))
        dx += dh_tilde_inner.reshape(1, -1).dot(self.Wx)  # (1, d)
        drh = self.Wh.T.dot(dh_tilde_inner)  # (h)
        # (h, h)
        self.dWh = dh_tilde_inner.reshape(-1,
                                          1).dot((self.r * self.hidden).reshape(1, -1))
        dh += (drh * self.r).reshape(1, -1)  # (1, h)
        dr = drh * self.hidden  # (h)
        dr_inner = dr * self.r_act.derivative()  # (h)
        # (h, h)
        self.dWrh = dr_inner.reshape(-1, 1).dot(self.hidden.reshape(1, -1))
        # (h, d)
        self.dWrx = dr_inner.reshape(-1, 1).dot(self.x.reshape(1, -1))
