...
class _BaseHMM():
    ...
    def __init__(self, n_state=1, x_size=1, iter=20):
        ...
        self.transmat_prob = np.ones(
            (n_state, n_state)) * (1.0 / n_state)  # 状态转换概率矩阵
    ...
    def predict(self, X, x_next, Z_seq=np.array([]), istrain=True):
        ...
        prob_x_next = self.emit_prob(
            np.array([x_next]))*np.dot(alpha[X_length - 1], self.transmat_prob)
        return prob_x_next
    ...
    def decode(self, X, istrain=True):
        ...
        max_pro_state = np.zeros(
            (X_length, self.n_state))  # 保存传递到序列某位置当前状态的最大概率

        _, c = self.forward(X, np.ones((X_length, self.n_state)))
        max_pro_state[0] = self.emit_prob(
            X[0]) * self.start_prob * (1/c[0])  # 初始概率
        ...
        for i in range(X_length):
            ...
            for k in range(self.n_state):
                prob_state = self.emit_prob(
                    X[i])[k] * self.transmat_prob[:, k] * max_pro_state[i-1]
    ...
    def train_batch(self, X, Z_seq=list()):
        ...
        for e in range(self.n_iter):  # EM步骤迭代
            ...
            b_post_adj_state = np.zeros(
                (self.n_state, self.n_state))  # 批量累积：相邻状态的联合后验概率，数组
            ...
            for n in range(X_num):  # 对于每个序列的处理
                ...
                post_adj_state = np.zeros(
                    (self.n_state, self.n_state))  # 相邻状态的联合后验概率
                for i in range(X_length):
                    if i == 0:
                        continue
                    if c[i] == 0:
                ...
                if np.sum(post_adj_state) != 0:
                    post_adj_state = post_adj_state / \
                        np.sum(post_adj_state)  # 归一化！
            ...
            self.emit_prob_updated(self.expand_list(
                X), self.expand_list(b_post_state))
    ...
    def forward(self, X, Z):
        ...
        for i in range(X_length):
            if i == 0:
                continue
            alpha[i] = self.emit_prob(
                X[i]) * np.dot(alpha[i - 1], self.transmat_prob) * Z[i]
            c[i] = np.sum(alpha[i])
            if c[i] == 0:
                continue
            alpha[i] = alpha[i] / c[i]
    ...
    def backward(self, X, Z, c):
        ...
        for i in reversed(range(X_length)):
            if i == X_length - 1:
                continue
            beta[i] = np.dot(
                beta[i + 1] * self.emit_prob(X[i + 1]), self.transmat_prob.T) * Z[i]
            if c[i+1] == 0:
                continue
            beta[i] = beta[i] / c[i + 1]
    ...
    def emit_prob_updated(self, X, post_state):  # 更新发射概率
        for k in range(self.n_state):
            for j in range(self.x_size):
                self.emit_means[k][j] = np.sum(
                    post_state[:, k] * X[:, j]) / np.sum(post_state[:, k])

            X_cov = np.dot(
                (X-self.emit_means[k]).T, (post_state[:, k]*(X-self.emit_means[k]).T).T)
...
class DiscreteHMM(_BaseHMM):
    ...
    def __init__(self, n_state=1, x_num=1, iter=20):
        ...
        self.emission_prob = np.ones(
            (n_state, x_num)) * (1.0/x_num)  # 初始化发射概率均值
        self.x_num = x_num

    def _init(self, X):
        self.emission_prob = np.random.random(size=(self.n_state, self.x_num))
        for k in range(self.n_state):
            self.emission_prob[k] = self.emission_prob[k] / \
                np.sum(self.emission_prob[k])
