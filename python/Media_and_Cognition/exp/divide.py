...
def divideTrainValiTest(source, target):
    ...
    for classes in pic_name:
        ...
        valid_list = pic_classes_name[int(
            0.8 * len(pic_classes_name)):int(len(pic_classes_name))]

        # 对于每个图片，移入到对应的文件夹里面
        for train_pic in train_list:
            shutil.copyfile(source + '/' + classes + '/' + train_pic,
                            target + '/train/' + classes + '/' + train_pic)
