...
class Model:
    ...
    def __init__(self, data_path, classes, config):
        ...
        self.data, self.data_name = load_images(data_path, labels=classes, is_train=self.is_train)
        if self.is_train:
        ...
        self.net5 = Net()
        if self.cuda_gpu:
            self.net1 = torch.nn.DataParallel(self.net1, device_ids=self.gpus).cuda()
            self.net2 = torch.nn.DataParallel(self.net2, device_ids=self.gpus).cuda()
            self.net3 = torch.nn.DataParallel(self.net3, device_ids=self.gpus).cuda()
            self.net4 = torch.nn.DataParallel(self.net4, device_ids=self.gpus).cuda()
            self.net5 = torch.nn.DataParallel(self.net5, device_ids=self.gpus).cuda()
        ...
        for i in range(batch_num):
            ...
            if self.is_train:
                for img, label in tmp_data:
                    ...
                    input_x1.append(self.transformer(img1).expand(1, 3, 32, 32))
                    input_x2.append(self.transformer(img2).expand(1, 3, 32, 32))
                    input_x3.append(self.transformer(img3).expand(1, 3, 32, 32))
                    input_x4.append(self.transformer(img4).expand(1, 3, 32, 32))
                    input_x5.append(self.transformer(img5).expand(1, 3, 32, 32))
            ...
            #print(input_x2.shape)
                ...
                if not_valid:
                    ...
                    output = (output1 + output2 + output3 + output4 + output5)/(5 * 5)
                #output = output.long()
    ...
    @staticmethod
    def distortion(img):    # distort the image in train epoches in order to get better generalize performance
        img_size = np.shape(img)
        resize_ratio = np.random.uniform(0.9, 1.1)  # get a random scale
        yshift = np.random.uniform(-0.1 * img_size[0], 0.1 * img_size[0])   # get a random shift amount
        xshift = np.random.uniform(-0.1 * img_size[1], 0.1 * img_size[1])
        M = np.float32([[1, 0, round(xshift)], [0, 1, round(yshift)]])
        img = cv2.warpAffine(img, M, (img_size[1], img_size[0]))    # random shift
        img = cv2.resize(img, None, fx=resize_ratio, fy=resize_ratio)   # random scale
        img_size = np.shape(img)
        rotate_ratio = np.random.uniform(-5, 5) # get a random degree
        M = cv2.getRotationMatrix2D((img_size[1] / 2, img_size[0] / 2), rotate_ratio, 1)    # get rotate matrix
    ...
    def train(self):
        ...
        for i in range(self.epoch_num):
            ...
            print('epoch '+str(int(self.epoch))+' finished, acc {}, loss {}'.format(correct[0]/total, loss/len(self.train_data)))
            print('Dataset accs:{} {} {} {} {}'.format(correct[1]/total,correct[2]/total,correct[3]/total,correct[4]/total,correct[5]/total))
            log_file.write('{}\tepoch '.format(time.strftime("%d-%H-%M"))+str(int(self.epoch))+'finished, acc {}, loss {}\n'.format(correct[0]/total, loss/len(self.train_data)))
            log_file.write('Dataset accs:{} {} {} {} {}\n'.format(correct[1]/total,correct[2]/total,correct[3]/total,correct[4]/total,correct[5]/total))
            log_file.flush()
            ...
            if (i+1) % self.valid_epochs == 0:
                ...
                print('Validation ended, acc {}, loss {}'.format(correct[0] / total, loss/len(self.valid_data)))
                print('Dataset accs:{} {} {} {} {}'.format(correct[1] / total, correct[2] / total, correct[3] / total,
                                                           correct[4] / total, correct[5] / total))
                log_file.write('{}\tValidation ended, acc {}, loss {}\n'.format(time.strftime("%d-%H-%M"), correct[0] / total, loss/len(self.valid_data)))
                log_file.write(
